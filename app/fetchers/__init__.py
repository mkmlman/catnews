...
async def fetch_all(client, fetchers: dict[str, FetchFn], limits: dict[str, int]) -> list[Story]:
    """Run all fetchers concurrently and interleave stories by source order."""
    import asyncio

    results = await asyncio.gather(*(fn(client) for fn in fetchers.values()), return_exceptions=True)
    stories: list[Story] = []
